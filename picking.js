...
class PickHelper {
  ...
  pick(normalizedPosition, scene, camera, time) {
    ...
    // const intersectedObjects = this.raycaster.intersectObjects(scene.children[0].children);
    var rootObject3D=scene.getObjectByName('rootObject3D');
    const intersectedObjects = this.raycaster.intersectObjects(rootObject3D.children);
    if (intersectedObjects.length) {
      // pick the first object. It's the closest one
      this.pickedObject = intersectedObjects[0].object;
      // save its color
      this.pickedObjectSavedColor = this.pickedObject.material.color.getHex();
      // set its color color to flashing red/yellow
...
class O_xyz {
  ...
  render(){
    ...
    let rafCallback=function(time){
      ...
      window.pickHelper.pick(f.pickPosition,f.scene,f.camera,time);
  ...
  getCanvasRelativePosition(ev){
    var f=this;
    var rect=f.canvas.getBoundingClientRect();
    return ({
      x:(ev.clientX-rect.left)*f.canvas.width/rect.width,
      y:(ev.clientY-rect.top)*f.canvas.height/rect.height
    });
    return f;
  }
  setPickPosition(ev){
    var f=this;
    var pos=f.getCanvasRelativePosition(ev);
    f.pickPosition.x=(pos.x/this.canvas.width)*2-1
    f.pickPosition.y=-((pos.y/this.canvas.height)*2-1)
